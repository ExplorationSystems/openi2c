...
export class MPU9250 extends Module {
    ...
    async init() {
        ...
        await this.bus.writeByte(MPU9250.MPU9250_ADDRESS, MPU9250.AK8963_CNTL, 0x00); // AK8963_CNTL register
        await this.bus.writeByte(MPU9250.MPU9250_ADDRESS, MPU9250.AK8963_CNTL, 0x0f); // AK8963_CNTL register
    ...
    async readGyroscope() {
        // Read gyroscope values, reads the low and high byte into a contiguous 16-bit value
        const x = await this.bus.readWord(MPU9250.MPU9250_ADDRESS, MPU9250.GYRO_XOUT_H);
        const y = await this.bus.readWord(MPU9250.MPU9250_ADDRESS, MPU9250.GYRO_YOUT_H);
        const z = await this.bus.readWord(MPU9250.MPU9250_ADDRESS, MPU9250.GYRO_ZOUT_H);

        return { x, y, z };
    }

    async getPitchAndRoll() {
        const { x, y, z } = await this.readGyroscope();
