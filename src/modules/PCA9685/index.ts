...
export class PCA9685 extends Module<Config> {
    ...
    async setFrequency(freq: number) {
        ...
        await this.bus.writeByte(this.address, this.config.MODE1, 0x10); // sleep
        await this.bus.writeByte(this.address, this.config.PRESCALE, prescale); // set frequency prescaler
        // Does it need to sleep?
        await sleep(1);
        await this.bus.writeByte(this.address, this.config.MODE1, 0x80); // wake up
        await sleep(1);
    }
    ...
    async setPWM(channel: number, on: number, off: number) {
        ...
        await Promise.all([
            this.bus.writeByte(this.address, this.config.LED0_ON_L + 4 * channel, on & 0xff),
            this.bus.writeByte(this.address, this.config.LED0_ON_H + 4 * channel, on >> 8),
            this.bus.writeByte(this.address, this.config.LED0_OFF_L + 4 * channel, off & 0xff),
            this.bus.writeByte(this.address, this.config.LED0_OFF_H + 4 * channel, off >> 8)
        ]);
    }
}
